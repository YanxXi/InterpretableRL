...
from collect_data import collect_dqn_data
import joblib
...
if __name__ == "__main__":
    ...
    parser = get_dqn_config(dqn_parser)
    args = parser.parse_args()

    # load deep rl model
    from algorithm.dqn import DQNPolicy
    dqn_policy = DQNPolicy(args, obs_space, act_sapce)
    dqn_policy.load_param('CartPole-v1', '05_16_11_01')

    print("\n-- Start Train Interpretable Model --\n")

    print("-- Collect Data By Reforcement Learnning Algorigtm --\n")

    data_set = collect_dqn_data(env, dqn_policy, 'action', M)
    ...
    while True:
        ...
        if MODE == "bc":
            ...
            new_data_set = collect_dqn_data(env, dqn_policy, 'action', M)
        ...
        if MODE == "dagger":
            ...
            new_data_set = label_data_set(dqn_policy, new_data_set)
    ...
    if MODE == "bc":
        np.save('cart_cartpole_avg2.npy', avg_reward_np)

    if MODE == "dagger":
        np.save('cart_cartpole_avg1.npy', avg_reward_np)

        joblib.dump(best_clf, 'cart_cartpole.pkl')
